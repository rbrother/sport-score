...
* Scoring table: laske calculations jo tulokset paremmin uutta taulukkoa vastaavaan muotoon
...
* [`src/accounts_rf/styles.cljs`](src/accounts_rf/styles.cljs): CSS compilation source file (ClojureScript,
[Garden](https://github.com/noprompt/garden))
* [`src/accounts_rf/`](src/accounts_rf/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/accounts_rf/core.cljs): contains the SPA entry point, `init`
...
5. Clone this repo and open a terminal in the project root directory
...
The `debug?` variable in [`config.cljs`](src/cljs/accounts_rf/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns accounts-rf.example
  (:require [accounts-rf.config :as config])
